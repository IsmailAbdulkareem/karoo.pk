...
export default function RateProviderScreen() {
  ...
  const { booking_id, provider_id, provider_name } = params
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      ...
      Alert.alert('✅ Shukriya!', 'Aapki rating submit ho chuki hai.', [
        { text: 'Okay', onPress: () => router.replace('/(user)/bookings') },
      ])
    } catch (err: any) {
      console.error(err)
    ...
    <ScrollView className="flex-1 bg-gray-950 px-6 py-10" contentContainerStyle={{ justifyContent: 'center' }}>
      <View className="bg-gray-900 border border-gray-800 rounded-3xl p-6 shadow-xl shadow-black/40">
        <View className="items-center mb-6">
          ...
          <Text className="text-white font-extrabold text-xl text-center">Rate Your Experience</Text>
        ...
        <View className="mb-6">
          ...
          <TextInput
            ...
            multiline
            numberOfLines={3}
            placeholder="Kaam ke baare mein apna tajurba share karein..."
